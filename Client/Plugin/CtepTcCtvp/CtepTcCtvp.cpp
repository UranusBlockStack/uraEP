...
VOID __stdcall CTVPVirtualChannelInitEventProc(LPVOID pInitHandle, UINT event, LPVOID pData, UINT dataLength)
{
	...
	g_log.Message(1, L"CTVPVirtualChannelInitEventProc ------------ \n");
	switch(event)
	{
	...
	case CHANNEL_EVENT_CONNECTED:
		{
			g_log.Message(1, L"CHANNEL_EVENT_CONNECTED ------------ \r\n");
	...
	case CHANNEL_EVENT_V1_CONNECTED:
		{
			g_log.Message(1, L"CHANNEL_EVENT_V1_CONNECTED ------------ \r\n");
		}
		break;

	case CHANNEL_EVENT_DISCONNECTED:
		{
			g_log.Message(1, L"CHANNEL_EVENT_DISCONNECTED ------------ \r\n");
	...
	case CHANNEL_EVENT_TERMINATED:
		{
			g_log.Message(1, L"CHANNEL_EVENT_TERMINATED ------------ \r\n");
			g_pCtvpTransClient->CloseChannelByInitHandle(pInitHandle);
			if ( g_fnClose)
			{
				g_fnClose();
	...
	default:
		{
			g_log.Message(1, L"SYSINF_C: unrecognized init event\r\n");
		}
		break;
	}
}

void WINAPI CTVPVirtualChannelOpenEvent(DWORD openHandle, UINT event
	, LPVOID pdata, UINT32 dataLength, UINT32 totalLength, UINT32 dataFlags)
{
	...
	default:
		{
			g_log.Message(1, L"SYSINF_C: unrecognized open event\r\n");
			//ASSERT(0);
		}
		break;
	}
