...
HRESULT CTransProIcaSvr::InitializeCompletePort(ICTEPTransferProtocolCallBack* piCallBack)
{
	...
	BOOL bRet = WFAPI::LoadFunction();
	if ( !bRet)
	{
		m_log.FmtError(3, L"加载ICA通道接口wfapi.dll失败!");
		return E_FAIL;
	}
...
HRESULT CTransProIcaSvr::PostRecv(CTransferChannel* pTransChn, ReadWritePacket* pPacket)
{
	...
	BOOL bRet = WFAPI::pWFVirtualChannelRead(hFile, 5000, pPacket->buff.buff, pPacket->buff.maxlength, &pPacket->ol.InternalHigh);
...
HRESULT CTransProIcaSvr::PostSend(CTransferChannel* pTransChn, ReadWritePacket* pPacket)
{
	...
	DWORD dwSent = 0;
	BOOL bRet = WFAPI::pWFVirtualChannelWrite(hFile, pPacket->buff.buff, pPacket->buff.size, &dwSent);
