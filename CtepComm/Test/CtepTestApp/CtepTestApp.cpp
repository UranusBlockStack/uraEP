...
public:
	...
	virtual DWORD    GetInterfaceVersion() override {return 1;} //默认返回0, ICTEPAppProtocolEx接口返回1,
	...
	virtual BOOL     QueryDisconnect(CUserData* pUser, CAppChannel *pAppChannel) override
	{
		return TRUE;
	}
	
	//Comm模块通知App模块指定通道的状态发生改变(Connected, Disconnected)
	virtual void     ChannelStateChanged(CUserData* pUser, CAppChannel *pAppChannel) override
	{
		;
	}
	...
	virtual BOOL     ConnectCrossApp(CUserData* pUser, CAppChannel* pNewAppChannel
		, CAppChannel *pStaticAppChannel) override
	{
		return FALSE;
	}

	virtual HRESULT  Connect(   CUserData* pUser, CAppChannel *pNewAppChannel
	...
	virtual HRESULT  ReadPacket(CUserData* pUser, CAppChannel *pAppChannel
